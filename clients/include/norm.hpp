...
inline double xlang(char* norm_type, int* m, int* n, rocblas_double_complex* A, int* lda, double* work)
{
    return zlange_(norm_type, m, n, A, lda, work);
}

inline float  xlanhe(char* norm_type, char* uplo, int* n, rocblas_float_complex* A, int* lda, float* work)
{
    return clanhe_(norm_type, uplo, n, A, lda, work);
}

inline float  xlanhe(char* norm_type, char* uplo, int* n, rocblas_double_complex* A, int* lda, double* work)
{
    return zlanhe_(norm_type, uplo, n, A, lda, work);
}

inline void xaxpy(int* n, float* alpha, rocblas_float_complex* x, int* incx, rocblas_float_complex* y, int* incy)
{
    ...
    char norm_type, rocblas_int M, rocblas_int N, rocblas_int lda, T* hCPU, T* hGPU)
{
    ...
    decltype(hCPU->x) work[1];
    rocblas_int incx        = 1;
    decltype(hCPU->x) alpha = -1.0f;
    rocblas_int size        = lda * N;
...
template <>
inline double norm_check_symmetric<rocblas_half, nullptr>(
    char norm_type, char uplo, rocblas_int N, rocblas_int lda, rocblas_half* hCPU, rocblas_half* hGPU)
{
    host_vector<double> hCPU_double(N * lda);
    host_vector<double> hGPU_double(N * lda);
